...
bool writeAll(int descriptor, std::string_view text, std::string &error)
{
    ...
    while (written < text.size()) {
        ...
        if (result < 0) {
            ...
            error = errorText("write macro table");
            return false;
        }
...
LoadResult Store::load()
{
    ...
    if (filesystem_error) {
        return {LoadDisposition::rejected, active_,
                "inspect macro table: " + filesystem_error.message()};
    }
    if (!exists) {
    ...
    if (filesystem_error) {
        return {LoadDisposition::rejected, active_,
                "inspect macro table size: " + filesystem_error.message()};
    }
    if (size > max_serialized_bytes) {
        return {LoadDisposition::rejected, active_,
                "macro table exceeds serialized size limit"};
    }
    errno = 0;
    std::ifstream stream(path_, std::ios::binary);
    if (!stream) {
        return {LoadDisposition::rejected, active_, errorText("read macro table")};
    }
    ...
    if (bytes_read < 0 ||
        ...
        return {LoadDisposition::rejected, active_, "read macro table failed"};
    }
    text.resize(static_cast<std::size_t>(bytes_read));
    char extra = 0;
    if (stream.get(extra)) {
        return {LoadDisposition::rejected, active_,
                "macro table changed or exceeds serialized size limit"};
    }
    ...
    return {decoded.migrated ? LoadDisposition::migrated
...
bool Store::save(const Snapshot &snapshot, std::string *error) const
{
    ...
    if (filesystem_error) {
        if (error) {
            *error = "create macro directory: " + filesystem_error.message();
        }
        return false;
    }
    ...
    if (descriptor < 0) {
        if (error) {
            *error = errorText("create temporary macro table");
        }
        return false;
    }
    ...
    if (!written && write_error.empty()) {
        write_error = errorText("sync macro table");
    }
    const int close_result = close(descriptor);
    if (!written || close_result != 0) {
        std::filesystem::remove(temporary_path, filesystem_error);
        if (error) {
            *error = write_error.empty() ? errorText("close macro table")
                                         : write_error;
        }
        return false;
    }
    if (rename(temporary_path.c_str(), path_.c_str()) != 0) {
        const std::string rename_error = errorText("replace macro table");
        std::filesystem::remove(temporary_path, filesystem_error);
        if (error) {
            *error = rename_error;
        }
    ...
    if (directory >= 0) {
        ...
        if (sync_result != 0) {
            if (error) {
                *error = errorText("sync macro directory");
            }
            return false;
        }
    }
